...
; Usage: KSPPLAY.COM FILE.KSP [SONG]
        ...
        ; Generated by tools/build_msx_dos2_ksp.py. These are entry/state
        ; addresses in the fixed page-3 player used by the page-0 helper.
...
PLAYER_RUNTIME_FILE_HANDLE: equ PLAYER_RUNTIME_RELOAD_ENTRY+2
PLAYER_RUNTIME_DIRECT_HELPER: equ PLAYER_RUNTIME_FILE_HANDLE+1
...
launch_player:
        ...
        ld      a,(file_handle)
        ld      (PLAYER_RUNTIME_FILE_HANDLE),a
        ld      hl,ksp_direct_helper
        ld      (PLAYER_RUNTIME_DIRECT_HELPER),hl
        ld      hl,sparse_reload_entry
        ld      (PLAYER_RUNTIME_RELOAD_ENTRY),hl

        ; Compact KSP uses its third allocated segment first as a temporary
        ; page-2 MWK uploader. The resident player clears it into MBWave work
        ; RAM immediately after the optional wavekit has been transferred.
        ld      a,(input_format)
        cp      2
        jr      nz,launch_player_no_mwk_blob
        ld      a,(kss_segments+2)
        call    mapper_put_p2
        ld      hl,mwk_uploader_blob
        ld      de,0x8000
        ld      bc,mwk_uploader_blob_end-mwk_uploader_blob
        ldir
        ld      a,(main_segments+2)
        call    mapper_put_p2
launch_player_no_mwk_blob:
        jp      0xC000
...
; Use whole-file staging whenever it fits. Compact KSP files use direct MWK
; streaming: only file segments containing the engine/music and directory are
; staged, while the MWK reader refills a 16 KiB page-1 buffer from DOS and
; writes samples directly to MoonSound RAM. Large compressed KCP files can be
...
choose_staging_mode:
        ...
        ld      a,(input_format)
        cp      2
        jr      nz,choose_staging_non_ksp
        ; Compact KSP keeps logical file segments 0, 1 and the final segment.
        ; The third runtime segment remains the MBWave work segment.
        ld      a,(declared_banks)
        add     a,3
        ld      b,a
        ld      a,(mapper_free)
...
; Compact KSP direct mode initially retains the first two 16 KiB source
; segments and the final segment containing KDIR/META. Runtime cache line 1
; can subsequently be refilled with any logical segment, so later SONG and
; MWK resources do not need to remain staged. MWK sample bytes are sent
; directly to OPL4 RAM.
stage_ksp_file:
        ld      a,(stage_pool)
        ld      hl,stage_segments
        ld      b,32
stage_ksp_fill_table:
        ld      (hl),a
        inc     hl
        djnz    stage_ksp_fill_table

        xor     a
        ld      c,a
        call    stage_ksp_segment
        ret     nz
        ld      a,(stage_count)
        cp      2
        jr      c,stage_ksp_done
...
; A = logical file segment, C = allocated stage-pool slot.
; The final logical segment is read only through EOF; all earlier segments
; are complete 16 KiB pages.
stage_ksp_segment:
        ...
        ; Compute logical_segment * 4000H in the 24-bit DOS file space.
        ld      a,(sparse_logical_segment)
        ld      c,a
        and     3
        rrca
        ...
        ; Read a short final page when the file does not end on a boundary.
        ld      hl,0x4000
        xor     a
        ld      (stage_read_final),a
        ld      a,(sparse_logical_segment)
        ld      b,a
        ld      a,(stage_count)
        dec     a
        cp      b
        jr      nz,stage_ksp_size_ready
        ld      hl,(file_size)
        ld      a,(file_size+2)
        ld      b,a
        ...
        ld      a,b
        sub     c
        ld      (stage_read_size+2),a
        ld      a,1
        ld      (stage_read_final),a
        ld      de,(sparse_source_offset)
        ld      hl,(file_size)
        or      a
        sbc     hl,de
        ld      (stage_read_size),hl
        jr      stage_ksp_seek
stage_ksp_size_ready:
        ld      (stage_read_size),hl
        xor     a
        ld      (stage_read_size+2),a
stage_ksp_seek:
        ...
        ld      b,a
        ld      de,0x8000
        ld      hl,(stage_read_size)
        ld      c,READ
        call    BDOS
        jr      z,stage_ksp_read_ok
        cp      EOF_ERROR
        jr      nz,stage_ksp_read_done
        ld      a,(stage_read_final)
        or      a
        jr      z,stage_ksp_read_done
stage_ksp_read_ok:
        xor     a
stage_ksp_read_done:
        ret

; DOS-backed compact-KSP helper retained in page 0.
; B=0: map logical file segment A into page 1.
; B=1: decode packed stream (A=destination segment, HL=size, DE=address).
; B=2: invalidate the cache after direct MWK reads.
ksp_direct_helper:
        ld      c,a
        ld      a,b
        or      a
        jr      z,ksp_direct_cache_segment
        cp      1
        jr      z,ksp_direct_decode_stream
        cp      2
        jr      z,ksp_direct_invalidate
        scf
        ret

ksp_direct_invalidate:
        ld      a,0xFF
        ld      (ksp_direct_cache_tag),a
        or      a
        ret

ksp_direct_cache_segment:
        ld      a,(ksp_direct_cache_tag)
        cp      c
        jr      z,ksp_direct_cache_map
        ld      a,c
        ld      (ksp_direct_requested),a
        ld      a,(stage_pool+1)
        call    mapper_put_p1

        ; DE:HL = logical segment * 4000H.
        ld      a,c
        and     3
        rrca
        rrca
        ...
        ld      c,READ
        call    BDOS
        jr      z,ksp_direct_cache_read_ok
        cp      EOF_ERROR
        jr      nz,ksp_direct_helper_fail
ksp_direct_cache_read_ok:
        ld      a,(ksp_direct_requested)
        ld      (ksp_direct_cache_tag),a
ksp_direct_cache_map:
        ld      a,(stage_pool+1)
        call    mapper_put_p1
        or      a
        ret

ksp_direct_decode_stream:
        ld      a,c
        ld      (ksp_direct_destination_segment),a
        ld      (ksp_direct_destination),de
        ld      (kcp_remaining),hl
        ld      hl,0x8000
        ld      (kcp_dest_offset),hl
        ld      a,(kss_segments+2)
        call    mapper_put_p2
        call    ksp_copy_raw_song_loop
        ld      a,(ksp_direct_destination_segment)
        call    mapper_put_p1
        ld      a,(kss_segments+2)
        call    mapper_put_p2
        ld      hl,0x8000
        ld      de,(ksp_direct_destination)
        call    kcpz_zx0_decoder
        or      a
        ret

ksp_direct_helper_fail:
        scf
        ret
...
stage_read_size: defs 3,0
stage_read_final:defb 0
ksp_direct_cache_tag:defb 0xFF
ksp_direct_requested:defb 0
ksp_direct_destination_segment:defb 0
ksp_direct_destination:defw 0
...
mwk_uploader_blob:
        incbin  'KSPDOS2_MWK.raw'
mwk_uploader_blob_end:
