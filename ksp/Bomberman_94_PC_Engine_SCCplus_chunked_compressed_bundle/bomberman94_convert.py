...
def read_tracks(m3u):
    def sec(s):
        if not s:return 0
        if ':' in s:
            a,b=s.split(':',1);return int(a)*60+int(b)
        return int(s)
    ...
    for line in open(m3u,encoding='utf-8-sig'):
        ...
        p=split_fields(line)
        if len(p)<4: raise ValueError(('bad m3u line',line,p))
        lf=p[4] if len(p)>4 else ''
        r.append({'id':int(p[1].lstrip('$'),16),'title':p[2],'duration':sec(p[3]),'loop':bool(lf)})
    return r

def convert_batch(start,end,outdir):
    hes='/mnt/data/bomberman94_hes/HC93065.hes';m3u='/mnt/data/bomberman94_hes/HC93065.m3u'; tracks=read_tracks(m3u)
    outdir=pathlib.Path(outdir);outdir.mkdir(parents=True,exist_ok=True)
    for i in range(start,min(end,len(tracks))):
        t=tracks[i]; trace=f'/tmp/bomberman94_{i}.trace'
        env=os.environ.copy();env['HES_TRACE_FILE']=trace;env['LD_PRELOAD']='/tmp/libhestrace_buffered.so'
