...
def compress_one(raw: bytes, tmp: Path, index: int) -> bytes:
    ...
    src.write_bytes(raw)
    proc = subprocess.run(
        ['java', '-jar', str(ZX0_JAR), '-f', str(src), str(dst)],
...
def process(pack: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix='kcpz_pack_') as td:
        tmp = Path(td)
        with concurrent.futures.ThreadPoolExecutor(max_workers=min(6, os.cpu_count() or 4)) as ex:
            packed_streams = list(ex.map(lambda t: compress_one(t[1], tmp, t[0]), enumerate(streams)))
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=min(6, os.cpu_count() or 4)) as ex:
            raw_streams = list(ex.map(lambda t: decompress_one(t[1], tmp, t[0]), enumerate(dec_inputs)))
