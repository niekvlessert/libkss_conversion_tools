...
import struct,json
KSP=Path('/mnt/data/parodius_sccplus_ksp/Parodius_Da_PC_Engine_SCCplus_prototype.ksp').read_bytes()
engine=Path('/mnt/data/parodius_sccplus_ksp/engine.bin').read_bytes(); emap=json.loads(Path('/mnt/data/parodius_sccplus_ksp/engine_map.json').read_text()); INIT=int(emap['labels']['init'],16);PLAY=int(emap['labels']['play'],16)
PTR=0x4700;WAIT=0x4702;ENDED=0x4703;TRACK=0x4800
class CPU:
 def __init__(self,track):
  self.m=bytearray(65536);self.m[0x4000:0x4000+len(engine)]=engine;self.m[TRACK:TRACK+len(track)]=track
  self.a=self.b=self.c=self.d=self.e=self.h=self.l=0;self.sp=0xF000;self.pc=0;self.z=False;self.ports={};self.psg_sel=0;self.steps=0
 ...
 def run_call(self,addr,maxsteps=200000):
  sentinel=0x0100;self.push(sentinel);self.pc=addr;start=self.steps
  while self.pc!=sentinel:
   if self.steps-start>maxsteps:raise RuntimeError(('runaway',hex(self.pc)))
   self.step()
 ...
 def step(self):
  ...
  elif op==0x3A:self.a=self.m[word()];self.z=(self.a==0) if False else self.z
...
def get_blocks():
 ...
 for _ in range(38):
  ...
  off,n=struct.unpack_from('<HH',KSP,pos);assert off==0x800;pos+=4
  yield KSP[pos:pos+n];pos+=n
...
def ref_event(b,s):
 ...
 for i in range(5):
  if wm>>i&1:
   wid=b[p];p+=1;s['mem'][32*i:32*i+32]=b[wavebase+wid*32:wavebase+(wid+1)*32]
 for i in range(5):
...
def compare(cpu,s,i,tick):
 if cpu.m[0xB800:0xB8B0]!=s['mem']:
  for j,(x,y) in enumerate(zip(cpu.m[0xB800:0xB8B0],s['mem'])):
   if x!=y:raise AssertionError((i,tick,'scc',hex(0xB800+j),x,y))
 for r,v in s['psg'].items():
  if cpu.ports.get(0x100+r,0)!=v:raise AssertionError((i,tick,'psg',r,cpu.ports.get(0x100+r,0),v))
 if cpu.m[WAIT]!=s['wait'] or cpu.m[ENDED]!=s['ended'] or cpu.rdw(PTR)!=TRACK+s['ptr']:
  raise AssertionError((i,tick,'state',cpu.m[WAIT],s['wait'],cpu.m[ENDED],s['ended'],hex(cpu.rdw(PTR)),hex(TRACK+s['ptr'])))

results=[]
for i,b in enumerate(get_blocks()):
 cpu=CPU(b);cpu.run_call(INIT);s=ref_init(b);compare(cpu,s,i,-1)
 frames=struct.unpack_from('<H',b,6)[0]
 for tick in range(frames+3):
  cpu.run_call(PLAY);ref_play(b,s);compare(cpu,s,i,tick)
 results.append({'track':i,'frames':frames,'steps':cpu.steps,'ended':cpu.m[ENDED],'loop':bool(b[4]&1)})
print(json.dumps({'ok':True,'tracks':len(results),'total_steps':sum(x['steps'] for x in results),'max_steps':max(x['steps'] for x in results),'results':results},indent=2))
