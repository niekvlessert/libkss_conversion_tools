...
static void make_kss_header(uint8_t prefix[KSP_KSS_HEADER_SIZE],
                            uint32_t load_size) {
  memset(prefix, 0, KSP_KSS_HEADER_SIZE);
...
int main(int argc, char **argv) {
  ...
  if (!ksp_compact_mwm(song.data, song.size, &song_image.data,
                       &song_image.size)) {
  ...
  prefix_size = KSP_KSS_HEADER_SIZE;
  prefix = (uint8_t *)malloc(prefix_size);
  if (!prefix) {
    fprintf(stderr, "out of memory\n");
    goto failure;
  }
  make_kss_header(prefix, load_size);
  ...
  printf("wrote %s: KSS load=4000H size=%04XH init=%04XH play=45E5H, %u chunks%s\n",
